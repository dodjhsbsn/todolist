  ...
  filterByYearMonth(String mode, int year, int month){
    ...
    if (mode == 'createTime'){
      for (int i = 1; i < 32; i++) {
        ...
        && DateTime.parse(element['createTime']).month == month).length);
      }
    }if (mode == 'finishedTime'){
      for (int i = 1; i < 32; i++) {
        sumTasksByYearMonth.add(tasksController.journals.where((element){
  ...
  List<BarChartGroupData> barGroups(String mode) => [
    for (int i = 0; i < 31; i++)
      BarChartGroupData(
        ...
        barRods: [
          BarChartRodData(
            toY: filterByYearMonth(mode, currentYear.value,currentMonth.value)[i].toDouble(),
            gradient: _barsGradient(mode),
          )
        ],
        showingTooltipIndicators: filterByYearMonth(mode, currentYear.value, currentMonth.value)[i].toDouble() != 0?[0]:[],
      ),
  ];
